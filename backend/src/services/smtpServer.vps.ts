...
class UltraZendSMTPServer {
  ...
  constructor(config: Partial<SMTPServerConfig> = {}) {
    ...
    logger.info('🔧 SMTPServer constructor - Config received:', config);
    ...
    logger.info('🔧 SMTPServer constructor - Final config:', this.config);
  ...
  private async validateRequiredTables(): Promise<void> {
    try {
      ...
      logger.info('SMTPServer.vps: Todas as tabelas obrigatórias validadas com sucesso');
    } catch (error) {
      logger.error('Erro ao validar tabelas do SMTPServer.vps:', error);
      throw error;
    }
  }
