...
const sslCertPaths = [
  {
    key: '/etc/letsencrypt/live/velomail.com.br/privkey.pem',
    cert: '/etc/letsencrypt/live/velomail.com.br/fullchain.pem',
    name: 'Let\'s Encrypt'
  },
  {
    key: '/etc/ssl/private/velomail.com.br.key',
    cert: '/etc/ssl/certs/velomail.com.br.crt', 
    name: 'Custom SSL'
  }
];
  ...
  try {
    ...
    if (Env.isProduction && httpsServer && !behindProxy) {
      ...
      httpsServer.listen(HTTPS_PORT, () => {
        ...
        logger.info(`📚 API Documentation: https://www.velomail.com.br/api-docs`);
      ...
      server.listen(PORT, () => {
        ...
        if (Env.isProduction) {
          logger.info(`📚 API Documentation: http://www.velomail.com.br:${PORT}/api-docs`);
