...
# UltraZend Professional Deploy Script (Node.js direto, sem Docker/Nginx)
set -euo pipefail
...
echo "🚀 UltraZend Professional Deployment"
echo "===================================="
...
ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_IP "
    ...
    npm run migrate:latest || echo 'Migrações falharam ou já estão atualizadas'
"
...
ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_IP "
    ...
    telnet localhost 25 < /dev/null && echo 'SMTP OK' || echo 'SMTP not responding'
"
...
echo "   Port: 25 (SMTP)"
echo ""
echo "🎯 Deployment completed with professional-grade simplified architecture:"
